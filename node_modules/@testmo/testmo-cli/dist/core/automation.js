...
var AutomationRunResults = /** @class */ (function () {
    ...
    AutomationRunResults.prototype.loadFromFiles = function (files, rewriterFile) {
        var _a, _b;
        ...
        if (rewriterFile) {
            logger_1.logger.debug("Trying to include rewriter script: ".concat(rewriterFile));
            var rewriterPath = path_1.default.resolve(rewriterFile);
            if (!rewriterPath || !fs_1.default.existsSync(rewriterPath)) {
                throw new Error("Rewriter script not found: ".concat(rewriterFile));
            }
            try {
                // eslint-disable-next-line global-require,import/no-dynamic-require
                var rewriter = require(rewriterPath);
                if (typeof rewriter.rewriteTest !== 'function') {
                    throw new Error('Rewriter function "rewriteTest" not found');
                }
                for (var _e = 0, _f = this.tests; _e < _f.length; _e++) {
                    var test_1 = _f[_e];
                    rewriter.rewriteTest(test_1);
                }
                logger_1.logger.debug('Rewriter script ran successfully');
            }
            catch (err) {
                if (err instanceof Error) {
                    logger_1.logger.error("Rewriter script returned an error: ".concat(err.message));
                }
                else {
                    logger_1.logger.error('Rewriter script returned an error');
                }
                throw err;
            }
        }
